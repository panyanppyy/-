...
struct twt_server {
	...
	int _sent(int x, string s, int len) {
		if (ok[x]) return 0;
...
struct twt_client {
	...
	int init(int x, string ip = "no") {
		initialization();
		server_addr.sin_family = AF_INET;
		if (ip == "no")	cout << "ip?\n> ", cin >> ip;
		if (ip == "o") ip = "127.0.0.1";
		if (ip.size() <= 3) ip = string("10.176.20.") + ip;
